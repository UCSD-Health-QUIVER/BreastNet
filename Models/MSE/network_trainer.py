...
class NetworkTrainer:
    ...
    def backward(self, output, target,body_mask):
        ...
        loss = self.setting.loss_function(output, target, body_mask)
        loss.backward()
    ...
    def prepare_batch(self,batch):
        ...
        if len(batch['CT'][tio.DATA].shape) == 4:
            ...
            batch['heart'][tio.DATA] = batch['heart'][tio.DATA].unsqueeze(0)
            batch['lung'][tio.DATA] = batch['lung'][tio.DATA].unsqueeze(0)
            batch['tumor'][tio.DATA] = batch['tumor'][tio.DATA].unsqueeze(0)
            batch['dose'][tio.DATA] = batch['dose'][tio.DATA].unsqueeze(0)
        inputs = torch.cat([
        batch['CT'][tio.DATA],
        batch['heart'][tio.DATA],
        batch['lung'][tio.DATA],
        batch['tumor'][tio.DATA],
        ],dim=1).float() 
    ...
    def train(self):
        ...
        for batch_idx, list_loader_output in enumerate(self.setting.train_loader):
            ...
            loss = self.backward(output, target, body_mask)
