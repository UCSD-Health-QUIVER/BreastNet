...
class AccrossConv(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(AccrossConv, self).__init__()
...
class UpConv(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.conv = nn.Sequential(
            nn.ConvTranspose3d(in_ch, out_ch, kernel_size=2, padding=0, stride=2, bias=True),
            # nn.Conv3d(in_ch, out_ch, kernel_size=3, padding=1, stride=1, bias=True),
            # nn.InstanceNorm3d(out_ch, affine=True),
            nn.BatchNorm3d(out_ch, affine=True),
            nn.LeakyReLU(inplace=True)
...
class Encoder(nn.Module):
    def __init__(self, in_ch, list_ch, dropout_prob):
        ...
        self.encoder_1 = nn.Sequential(
            SingleConv(in_ch, list_ch[1], kernel_size=3, stride=1, padding='same'),
            SingleConv(list_ch[1], list_ch[1], kernel_size=3, stride=1, padding='same'),
            torch.nn.Dropout3d(p=dropout_prob),
       
        )
        self.encoder_2 = nn.Sequential(
            SingleConv(list_ch[1], list_ch[2], kernel_size=3, stride=1, padding=1),
            SingleConv(list_ch[2], list_ch[2], kernel_size=3, stride=1, padding="same"),
            torch.nn.Dropout3d(p=dropout_prob),
           
        )
        self.encoder_3 = nn.Sequential(
            SingleConv(list_ch[2], list_ch[3], kernel_size=3, stride=1, padding=1),
            SingleConv(list_ch[3], list_ch[3], kernel_size=3, stride=1, padding='same'),
            torch.nn.Dropout3d(p=dropout_prob),
        ...
        self.encoder_6 = nn.Sequential(
            SingleConv(list_ch[5], list_ch[6], kernel_size=3, stride=1, padding=1),
            SingleConv(list_ch[6], list_ch[6], kernel_size=3, stride=1, padding=1),
            torch.nn.Dropout3d(p=dropout_prob),
      

        )
        self.MaxPool = torch.nn.MaxPool3d((2,2,2))
        

    def forward(self, x):
        out_encoder_1 = self.encoder_1(x)
        x = self.MaxPool(out_encoder_1)

        out_encoder_2 = self.encoder_2(x)
        x = self.MaxPool(out_encoder_2)

        out_encoder_3 = self.encoder_3(x)
        x = self.MaxPool(out_encoder_3)

        out_encoder_4 = self.encoder_4(x)
        x = self.MaxPool(out_encoder_4)

        out_encoder_5 = self.encoder_5(x)
        x = self.MaxPool(out_encoder_5)

        out_encoder_6 = self.encoder_6(x)

        return [out_encoder_1, out_encoder_2, out_encoder_3, out_encoder_4, out_encoder_5, out_encoder_6]
...
class Decoder(nn.Module):
    def __init__(self, list_ch, dropout_prob):
        ...
        self.upconv_5 = UpConv(list_ch[6], list_ch[5])
        ...
        self.decoder_conv_1 = nn.Sequential(
            ...
            SingleConv(list_ch[1], list_ch[1]//2, kernel_size=3, stride=1, padding=1),
            SingleConv(list_ch[1]//2, list_ch[1]//4, kernel_size=3, stride=1, padding=1),
   
        )

    def forward(self, out_encoder):
        out_encoder_1, out_encoder_2, out_encoder_3, out_encoder_4, out_encoder_5, out_encoder_6 = out_encoder

        out_decoder_5 = self.decoder_conv_5(
            torch.cat((self.upconv_5(out_encoder_6), out_encoder_5), dim=1)   
        )
        out_decoder_4 = self.decoder_conv_4(
            torch.cat((self.upconv_4(out_decoder_5), out_encoder_4), dim=1)
        )
...
class Model(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, list_ch_A,dropout_prob =0.0):
        ...
        self.conv_out_A = nn.Conv3d(list_ch_A[1]//4, out_ch, kernel_size=1, padding=0, bias=True)
